...
public class NestedScrollingScaleParent2Layout extends LinearLayout implements NestedScrollingParent2 {
    ...
    public NestedScrollingScaleParent2Layout(Context context) {
        this(context, null);
    }

    public NestedScrollingScaleParent2Layout(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NestedScrollingScaleParent2Layout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        setOrientation(VERTICAL);
        flagY=dip2px(context,50);
    }
    ...
    public void onNestedPreScroll(@NonNull View target, int dx, int dy, @NonNull int[] consumed, int type) {
        ...
        //这里不管手势滚动还是fling都处理
        int currentHeight = mTopView.getHeight();
        boolean hideTop = dy > 0 && currentHeight > flagY ;
        boolean showTop = dy < 0
                && currentHeight < mTopViewHeight
        ...
        //对于底部布局
        boolean hideBottom = dy < 0 && currentHeight > mTopViewHeight;
...
//            scrollBy(0, dy);
            ...
            params.height-=dy;
            if (params.height<flagY){
                params.height=flagY;
            }
            if (params.height>mTopViewHeight){
                params.height=mTopViewHeight;
            }
